...
export default function FacultyDashboard() {
    const [activeTab, setActiveTab] = useState<"sync" | "schedule">("sync");
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-slate-400">Manage your academy rosters and live session schedule.</p>
                    </div>
                </div>
                ...
                <div className="flex bg-slate-900/50 p-1 rounded-2xl border border-slate-800">
                    <button
                        ...
                        className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === "sync" ? "bg-indigo-600 text-white shadow-lg" : "text-slate-400 hover:text-white"
                            }`}
                    >
                        Google Sync
                    </button>
                    <button
                        ...
                        className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === "schedule" ? "bg-indigo-600 text-white shadow-lg" : "text-slate-400 hover:text-white"
                            }`}
                    >
                        Live Schedule
                    </button>
                            ...
                            <div className="space-y-3 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                                {roster.length > 0 ? roster.map((student, idx) => (
                                    <div key={idx} className="flex items-center gap-3 p-3 bg-slate-950/50 rounded-lg border border-slate-800/50">
                                        <div className="w-8 h-8 rounded-full bg-indigo-500/20 flex items-center justify-center text-indigo-400 font-bold text-xs">{student.profile.name.fullName[0]}</div>
                                        <div>
                        ...
                        <div className="bg-slate-900/40 backdrop-blur-xl border border-slate-800 p-10 rounded-3xl h-full">
                            ...
                            <form onSubmit={handlePostHomework} className="space-y-8">
                                ...
                                <div className="pt-4 flex justify-end">
                                    <button type="submit" disabled={isPosting} className="group flex items-center gap-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold px-10 py-5 rounded-2xl transition-all shadow-xl shadow-indigo-600/20 active:scale-95 disabled:opacity-50">
